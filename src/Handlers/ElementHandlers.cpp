 ...
 * @return std::shared_ptr<Tag>
 */
std::shared_ptr<PrologTag> ElementHandlers::buildPrologTag(
    std::vector<std::shared_ptr<Component>> &components,
    size_t &m_index,
    size_t &start)
{
    std::shared_ptr<OpenPrologComponent> openTag = std::dynamic_pointer_cast<OpenPrologComponent>(components[start]);
    ComponentUtilities::IncrementIndex(components, m_index);
    ComponentUtilities::IgnoreWhiteSpace(components, m_index);
    std::shared_ptr<Name> tagName = std::dynamic_pointer_cast<Name>(components[m_index]);
    ComponentUtilities::IncrementIndex(components, m_index);
    ...
    if (components[m_index]->type() == ComponentType::ClosePrologComponent)
    {
        std::shared_ptr<ClosePrologComponent> closeTag = std::dynamic_pointer_cast<ClosePrologComponent>(components[m_index]);
        ComponentUtilities::IncrementIndex(components, m_index);
        ComponentUtilities::IgnoreWhiteSpace(components, m_index);
        return std::make_shared<PrologTag>(openTag, tagName, props, closeTag);
    }
